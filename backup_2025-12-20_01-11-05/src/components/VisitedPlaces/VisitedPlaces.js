...
// Mantendo todas as funções do Firestore necessárias
...
function VisitedPlaces({ onBack }) {
    ...
    async function handleSave() {
        ...
        if (editingId) {
            const placeRef = doc(db, "places", editingId);
            await updateDoc(placeRef, { name, category, rating, notes });
            setPlaces(places.map(p => p.id === editingId ? { ...p, name, category, rating, notes } : p));
            setEditingId(null);
        } else {
            const docRef = await addDoc(collection(db, "places"), { name, category, rating, notes });
            setPlaces([...places, { id: docRef.id, name, category, rating, notes }]);
        }
    ...
    async function confirmDelete() {
        await deleteDoc(doc(db, "places", deleteId));
