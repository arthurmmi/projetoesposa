...
function TravelIdeas({ onBack }) {
    ...
    const fetchTrips = async () => {
        try {
            const tripsData = await api.getTravelIdeas();
            setTrips(tripsData);
        } catch (error) {
            console.error("Erro ao buscar viagens:", error);
            alert("Erro ao carregar as viagens. Verifique se o servidor backend está rodando.");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDeleteTrip = async (trip) => {
        ...
        try {
            await api.deleteTravelIdea(trip.id);
